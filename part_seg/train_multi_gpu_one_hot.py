...
#  ShapeNet :  8326770  50.5M
#  S3DIS :    11512845  
#                       17.1MB
#  ScanNet :  11459413  131 MB
#              1129063
#  ModelNet40 : 599340
...
parser.add_argument('--num_gpus', type=int, default=2, help='How many gpus to use [default: 2]')
parser.add_argument('--model', default='dgs_part_seg_one_hot', help='Model name [default: model]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/cpu:0'):
            ...
            # Get model and loss 
            MODEL.get_model(pointclouds_pl, cls_labels_pl, is_training_pl, bn_decay=bn_decay)
            tower_grads = []
            pred_gpu = []
            total_loss_gpu = []
            for i in range(NUM_GPUS):
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    with tf.device('/gpu:%d'%(i)), tf.name_scope('gpu_%d'%(i)) as scope:
                        ...
                        cls_labels_pl_batch = tf.slice(cls_labels_pl,
                            [i*DEVICE_BATCH_SIZE],[DEVICE_BATCH_SIZE])
                        label_batch = tf.slice(labels_pl,
                            [i*DEVICE_BATCH_SIZE,0], [DEVICE_BATCH_SIZE, -1])

                        pred, end_points = MODEL.get_model(pc_batch, cls_labels_pl_batch, is_training_pl, bn_decay=bn_decay)
                        print(pred)
                        loss = MODEL.get_loss(pred, label_batch)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    total_seen = 0
    loss_sum = 0
    print("num of param:", get_num_params())
