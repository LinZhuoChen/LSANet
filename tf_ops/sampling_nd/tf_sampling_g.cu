...
__global__ void gatherpointKernel(int b,int n,int m,int d,const float * __restrict__ inp,const int * __restrict__ idx,float * __restrict__ out){
  for (int i=blockIdx.x;i<b;i+=gridDim.x){
    for (int j=blockIdx.y*blockDim.x+threadIdx.x;j<m;j+=blockDim.x*gridDim.y){
      int a=idx[i*m+j];
      // allows for input points to have n-d features
      for(int k = 0; k < d; k++)
        out[(i*m+j)*d+k]=inp[(i*n+a)*d+k];
    }
  }
}

__global__ void scatteraddpointKernel(int b,int n,int m,int d,const float * __restrict__ out_g,const int * __restrict__ idx,float * __restrict__ inp_g){
  for (int i=blockIdx.x;i<b;i+=gridDim.x){
    for (int j=blockIdx.y*blockDim.x+threadIdx.x;j<m;j+=blockDim.x*gridDim.y){
      int a=idx[i*m+j];
      // allows for input points to have n-d features
      for(int k = 0; k < d; k++)
          atomicAdd(&inp_g[(i*n+a)*d+k],out_g[(i*m+j)*d+k]);
    }
  }
}
...
void gatherpointLauncher(int b,int n,int m,int d,const float * inp,const int * idx,float * out){
  gatherpointKernel<<<dim3(2,8,1),512>>>(b,n,m,d,inp,idx,out);
}
void scatteraddpointLauncher(int b,int n,int m,int d,const float * out_g,const int * idx,float * inp_g){
  scatteraddpointKernel<<<dim3(2,8,1),512>>>(b,n,m,d,out_g,idx,inp_g);
}
