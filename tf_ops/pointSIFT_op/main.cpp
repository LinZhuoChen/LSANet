...
REGISTER_OP("CubeSelectEight")
        ...
        .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
            ::tensorflow::shape_inference::ShapeHandle dim1;
            c->WithRank(c->input(0), 3, &dim1); // batch_size * npoint * 3
            ::tensorflow::shape_inference::ShapeHandle output1 = c->MakeShape({c->Dim(dim1, 0), c->Dim(dim1, 1), 64});
            c->set_output(0, output1); // batch_size * npoint * 64
            return Status::OK();
        });
...
public:
    ...
    void Compute(OpKernelContext* context) override {
        ...
        auto idx_out_flat = idx_out_tensor->flat<int>();
        int* idx_out = &(idx_out_flat(0));
        cubeSelectEightLauncher(b, n, radius_, xyz, idx_out);
    }
private:
    float radius_;
};
