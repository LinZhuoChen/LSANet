...
__global__ void cube_select_eight(int b, int n,float radius, const float* xyz, int* idx_out) {
    int batch_idx = blockIdx.x;
    xyz += batch_idx * n * 3;
    idx_out += batch_idx * n * 64;
    float temp_dist[64];
    float judge_dist = radius * radius;
    for(int i = threadIdx.x; i < n;i += blockDim.x) {
        float x = xyz[i * 3];
        ...
        for(int j = 0;j < 64;j ++) {
            temp_dist[j] = judge_dist;
            idx_out[i * 64 + j] = i; // if not found, just return itself..
        }
        for(int j = 0;j < n;j ++) {
            if(i == j) continue;
            float tx = xyz[j * 3];
            ...
            for(int k = 0;k < 8;k ++) {
                ...
                if (flag) {
                    for (int kk = 7; kk >= k + 1; kk --) {
                        idx_out[i * 64 + temp_idx + kk] = idx_out[i * 64 + temp_idx + kk - 1];
                        temp_dist[temp_idx + kk] = temp_dist[temp_idx + kk - 1];
                    }
                    idx_out[i * 64 + temp_idx + k] = j;
                    temp_dist[temp_idx + k] = dist;
                    break;
                }
            }
...
void cubeSelectEightLauncher(int b, int n, float radius, const float* xyz, int* idx_out) {
    cube_select_eight<<<b, 512>>>(b, n, radius, xyz, idx_out);
}
