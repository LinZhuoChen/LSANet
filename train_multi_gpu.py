...
def get_learning_rate(batch):
    learning_rate = tf.train.exponential_decay(
                        ...
                        staircase=True)
    learning_rate = tf.maximum(learning_rate, 0.00002) # CLIP THE LEARNING RATE!
...
def train():
    with tf.Graph().as_default():
            ...
            for i in range(NUM_GPUS):
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    with tf.device('/gpu:%d'%(i)), tf.name_scope('gpu_%d'%(i)) as scope:
                        ...
                        total_loss = MODEL.get_loss(pred, label_batch, end_points)
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if acc > 0.919:
                save_path = saver.save(sess, os.path.join(LOG_DIR, "model_acc_%f.ckpt"%(acc)))
                log_string("Model saved in file: %s" % save_path)
